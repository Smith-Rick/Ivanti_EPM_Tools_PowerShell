...
<#	
	...
	===========================================================================
	.DESCRIPTION
		Automate the execution of an LANDesk\EPM provisioning template for the device using the "Quick Seal" process.
#>


#Template details for the task
$templateName = '_VDI - RUN MaintMode QuickSeal Condensed'
...
{
	New-PSDrive -Name X -PSProvider FileSystem -Root \\<UNC PATH HERE> -Credential $mycreds
	Copy-Item "\\<UNC PATH HERE>\Microsoft\Sysinternals\PsExec.exe" $psExecPath
